...
Starting with Gradle 9.0, the minimum supported Android Gradle Plugin version is 7.4.0.
Earlier versions are no longer supported as they rely on Gradle APIs that have been removed.
...
Earlier versions are no longer supported as they rely on Gradle APIs that have been removed.
Consider upgrading to the latest version of the Gradle Enterprise Plugin, or even better to the latest version of the Develocity Plugin.

==== Conventions removal

The "convention" concept, defined by the `org.gradle.api.plugins.Convention` type, has been deprecated since Gradle 8.2 and is now removed in Gradle 9.0.

Core Gradle plugins that still registered deprecated conventions have also been updated, leading to their removal:

.Removed plugin extensions
[cols="1,1,1,5"]
|===
